...
public class EventManager<TEventType> : EventManager
    where TEventType : struct, IComparable, IConvertible, IFormattable
{
    ...
    public override bool SaveAllEvents
    {
        get { return saveAllEvents; }
        set { saveAllEvents = value; }
    }
    public override bool HasPersistentEvents
    {
    ...
    public EventManager()
    {
        if (!typeof(TEventType).IsEnum)
            throw new InvalidGenericTypeException(
                "TEventType for generic class 'Event Manager' must be an enum!"
            );
    ...
    public void MarkEventAsPersistent(TEventType eventName)
    {
     ...
     //   Debug.Log("GlobalEventManager MarkAsPermanent \t\ eventID: " + eventID + "\"");
        ...
        if (persistentEvents.Add(eventID))
            hasPersistentEvents = true;
    }
    ...
    public override void RemoveInterimEvents()
    {
#if LOG_ALL_MESSAGES
        Debug.Log(
            "MESSENGER RemoveInterimEvents. Make sure that none of necessary listeners are removed."
        );
#endif
        ...
        foreach (int eventID in persistentEvents)
        {
            tempGlobalEventHolder[eventID] = eventHandlers[eventID];
        }
    ...
    public void PrintGlobalEventTable()
    {
        ...
        for (int i = 0; i < eventHandlers.Length; i++)
            Debug.Log(
                "Event Name: " + (TEventType)(object)i + "  ||  Delegate Type: " + eventHandlers[i]
            );
    ...
    private void OnListenerRemoving(int eventID, Delegate listenerBeingRemoved)
    {
        ...
        if (tempDel == null)
        {
            throw new ListenerException(
                string.Format(
                    "Attempting to remove listener for event type \"{0}\" but current listener is null.",
                    (TEventType)(object)eventID
                )
            );
        }
    ...
    static public void OnBroadcasting(int eventID)
    {
        if (!eventHandlers.ContainsKey(eventID))
        {
            throw new BroadcastException(
                int.Format(
                    "Broadcasting message \"{0}\" but no listener found. Try marking the message with GlobalEventManager.MarkAsPermanent.",
                    (TEventType)(object)eventID
                )
            );
        }
    ...
    //Single parameter
    public IEnumerator Broadcast<T>(
        TEventType eventName,
        T arg1,
        YieldInstruction[] delayInstrunctions
    )
    {
        int eventID = ConvertEnumToInt(eventName);
#if LOG_ALL_MESSAGES || LOG_BROADCAST_MESSAGE
    ...
    //Two parameters
    public IEnumerator Broadcast<T, U>(
        TEventType eventName,
        T arg1,
        U arg2,
        YieldInstruction[] delayInstrunctions
    ...
    //Three parameters
    public IEnumerator Broadcast<T, U, V>(
        TEventType eventName,
        T arg1,
        U arg2,
        V arg3,
