...
public sealed class EventManagerRepository
{
            ...
            if (instance == null) //If instance hasn't been found/created yet
            {
                lock (padlock)
                {
                    if (instance == null)
                    {
                        ...
                        EventManagerRepositoryCleaner cleaner = (EventManagerRepositoryCleaner)
                            Object.FindObjectOfType(typeof(EventManagerRepositoryCleaner));
                        if (cleaner == null)
                            (new GameObject("Repository Cleaner"))
                                .AddComponent<EventManagerRepositoryCleaner>()
                                .Initialize(Instance.CleanupRepository);
                    }
                }
            }
            return instance;
    ...
    public EventManager<TEventType> GetSingleInstanceEventManager<TEventType>()
